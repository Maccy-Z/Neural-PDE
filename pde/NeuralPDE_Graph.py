...
class NeuralPDEGraph:
    ...
    def __init__(self, pde_fn: PDEFunc, cfg: Config, loss_fn:Loss = None):
        self.cfg = cfg
    ...
    def single_step(self, U_graph: UGraph, Us_current: UValues, Us_true: UValues):
        ...
        with torch.no_grad():
            init_loss = self.loss_fn(Us_current, Us_true, requires_grad=False)
            final_loss = self.loss_fn(Us_new, Us_true, requires_grad=False)
