...
class BCCalc:
    def __init__(self, bc_specs: dict[int, list[Deriv]], N_comp: int, N_Us: int, dif_degrees: list[tuple],
        ...
        self.N_idx = len(lens)          # = N_bc * N_comp
        assert len(comps) == self.N_idx and len(weights) == self.N_idx
        assert self.N_idx == self.N_bc * self.N_comp
        # concat ragged lists
        ...
        self.batch_ids = torch.repeat_interleave(torch.arange(self.N_idx, device=device), lens)  # shape = [N_idx]
        ...
        # Make Jacobian for BCs: J = dR/dU_dUs
        # 4-D indices for (b, c_out, o, k)
        indices = torch.stack([self.batch_ids, self.comps_cat, self.orders_cat, self.comps_cat])  # [4, N_idx]
        size = (self.N_bc, self.N_comp, N_derivs, self.N_comp)
        # Sparse Jacobian (coalesced sums duplicate entries automatically)
        J = torch.sparse_coo_tensor(indices, self.weights_cat, size, device=self.device).coalesce().to_dense()        # shape = [N_bc, N_comp, N_derivs, N_comp]
        self.J = J.reshape(self.N_bc * self.N_comp, N_derivs * self.N_comp)   # shape = [N_bc*N_comp, N_derivs*N_comp]
    ...
    def forward(self, U_dUs: torch.Tensor):
        ...
        out = torch.zeros(self.N_idx, device=self.device)
